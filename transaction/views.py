...
from transaction.db import q_nasabah, q_nasabah_kolektif, q_vendor, q_pembelian, q_penjualan, q_konversi, q_nasabah_detail, q_vendor_detail, q_penjualan_detail, q_pembelian_detail, q_konversi_detail, q_remaining, q_get_last_stock, q_home, q_is_pembelian_clear , q_is_konversi_clear, q_laba_rugi, q_nasabah_all, q_arus_barang
...
@login_required()
def nasabah(request):
	context = { 'nasabah': q_nasabah, 'user': request.user}
	return render(request, 'transaction/nasabah.html', context)

@cache_control(no_cache=True, must_revalidate=True, no_store=True)
@login_required()
def nasabah_kolektif(request):
	context = { 'nasabah': q_nasabah_kolektif, 'user': request.user}
